...
for C_i in range(200,201):
    ...
    for label_i in unique_labels:
        if label_i != label_ct and label_i != 1:            
            if (label_graph[label_i][label_ct]['border'] > 10) and (label_graph.nodes[label_i]['volume'] < label_graph.nodes[label_ct]['volume']*0.8):
    ...
    for label_i in unique_labels:        
        if np.count_nonzero(label_chosen==label_i) == 0 and label_i != 1:
            ...
            for label_chosen_i in label_chosen[1:label_chosen_size]:
                if (label_graph[label_i][label_chosen_i]['border'] > 10) and (label_graph.nodes[label_i]['volume'] < label_chosen_volume_prv *0.8):
                    label_chosen = np.append(label_chosen,[label_i])
                    label_chosen_volume += label_graph.nodes[label_i]['volume']
    ...
    for label_i in unique_labels:        
        if np.count_nonzero(label_chosen==label_i) == 0 and label_i != 1:
            # only go through all blobs from the previous round and not the centre blob, and only compare with the previous volume
            for label_chosen_i in label_chosen[1:label_chosen_size]:
                if (label_graph[label_i][label_chosen_i]['border'] > 10) and (label_graph.nodes[label_i]['volume'] < label_chosen_volume_prv *0.3):
                    label_chosen = np.append(label_chosen,[label_i])
                    label_chosen_volume += label_graph.nodes[label_i]['volume']
    ...
    for label_i in unique_labels:        
        if np.count_nonzero(label_chosen==label_i) == 0 and label_i != 1:
            # only go through all blobs from the previous round and not the centre blob, and only compare with the previous volume
            for label_chosen_i in label_chosen[1:label_chosen_size]:
                if (label_graph[label_i][label_chosen_i]['border'] > 10) and (np.shape(label_graph.nodes[label_i]['neighbors'])[0]==2) and (label_graph.nodes[label_i]['volume'] < label_chosen_volume_prv *0.3):
                    label_chosen = np.append(label_chosen,[label_i])
                    label_chosen_volume += label_graph.nodes[label_i]['volume']
...
#%% Plot stat images with 4 subfigures
    ...
    im = plt.imshow(C_binary8_second, extent = (lon_min, lon_max, lat_min, lat_max),  cmap='Greys',origin='lower')
    # Best track center
    ...
    im = plt.imshow(C_binary8_filled8, extent = (lon_min, lon_max, lat_min, lat_max),  cmap='Greys_r',origin='lower')
    ax.set_title(filename)
    ax.set_xlabel('Longitude')
    ax.set_ylabel('Latitude')
...
#    fig.savefig("stat_"+SAVDIR + "\\" + filename +".png",dpi=1000)
#    plt.close()
    plt.show()    
...
#%%
    ...
    plt.subplot(122) #with holes filled
...
#%% plot IR image and the center point
    ...
    ax.set_xlabel('Longitude')
    ax.set_ylabel('Latitude')
    fig.savefig("f_"+SAVDIR + "\\" + filename +".png",dpi=1000)
    plt.close()
#    plt.show()            
