...
SAVDIR = WORKPLACE + r"\3_Figures\DORIAN"
...
#for C_i in range(0,1):
    ...
    distance = ndimage.distance_transform_edt(C_binary8)
    local_maxi = peak_local_max(distance, indices=False, footprint=np.ones((3, 3)), labels=C_binary8)
    markers = measure.label(local_maxi)
    labels_ws = watershed(-distance, C_flag, mask=C_binary8)
...
#plt.subplot(222)
##% Plot BT image with 3 labels
#im = plt.imshow(distance, extent = (lon_min, lon_max, lat_min, lat_max),  cmap='Greys_r',origin='lower')
#plt.subplot(223)
#im = plt.imshow(markers, extent = (lon_min, lon_max, lat_min, lat_max),  cmap='Greys_r',origin='lower')
#plt.subplot(224)
#im = plt.imshow(labels_ws, extent = (lon_min, lon_max, lat_min, lat_max),cmap='Spectral',origin='lower')
## Best track center
#plt.plot(I_lon[C_i],I_lat[C_i],'or', markersize = 2) 
#ax = plt.gca()
#ax.set_title(filename)
