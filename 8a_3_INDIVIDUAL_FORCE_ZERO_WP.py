...
CODEDIR = r"C:\Users\z3439910\Documents\Kien\1_Projects\2_Msc\1_E1\5_GIS_project\1_Pythoncodes\TCcloudseg"
os.chdir(CODEDIR)
import TC_funtions
...
#%%
i_value_in_year = 999
chosen_TC_serial = "2012162N06150"
for i,j in enumerate(TC_serials):
    if j == chosen_TC_serial:
        i_value_in_year = i
#print(i_value_in_year)
...
#%
SAVDIR = WORKPLACE + r"\3_Figures\\" + r"WP\\" + TC_serial + "_" + I_name
...
print("C_i_start = " + str(C_i_start))
#%%
step_to_fix = 84
for C_i in range(step_to_fix,step_to_fix+1):
...
#%% CONTINUE SPREADING
    step_to_fix = 115
for C_i in range(step_to_fix+1,DIM_TIME):
        C_label_TC[C_i,:,:] = np.zeros([DIM_LAT,DIM_LON])
        BTemp_filename = r"merg_"+ time_to_string_without_min(I_year[C_i],I_month[C_i],I_day[C_i],I_hour[C_i]) + r"_4km-pixel.nc4"
    ...
    #        else:
        C_i_previous = C_i-1
        if C_label_TC[C_i_previous,:,:].nonzero()[1].shape[0] == 0:
            C_i_previous = C_i_previous - 1
        C_flag_prev = C_label_TC[C_i_previous,:,:][:] 
    ...
    #%de')
        ...
        #% Compare to eliminate error frame
        if C_flag.nonzero()[1].shape[0] < C_label_TC[C_i_previous,:,:].nonzero()[1].shape[0]*0.5:
            C_flag = np.zeros([DIM_LAT,DIM_LON])
