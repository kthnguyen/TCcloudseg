...
if I_minute == 0:
    ...
    C_BTemp = xr.open_dataset(IRDIR+ "\\" + BTemp_filename)['Tb'].values[0]
    C_lat = xr.open_dataset(IRDIR+ "\\" + BTemp_filename)['lat'].values
    C_lon = xr.open_dataset(IRDIR+ "\\" + BTemp_filename)['lon'].values
...
#    C_BTemp[mask] = np.interp(np.flatnonzero(mask), np.flatnonzero(~mask), C_BTemp[~mask])
    ...
    #slice out BT images for the current basin
    C_BTemp = xr.open_dataset(IRDIR+ "\\" + BTemp_filename)['Tb'].values[1]
    C_lat = xr.open_dataset(IRDIR+ "\\" + BTemp_filename)['lat'].values
    C_lon = xr.open_dataset(IRDIR+ "\\" + BTemp_filename)['lon'].values
#    #interpolate NaN values in BT images
#    mask = np.isnan(C_BTemp)
